...
const UserSchema = new mongoose.Schema(
  {
    ...
    // Total portfolio capital (USD). Buys don't change it;
    // realized sell P&L settles here (+profit / -loss).
    // Cash left is derived as totalAsset - invested cost.
    totalAsset: {
      type: Number,
      default: 0,
    },
    ...
    // Telegram integration — links a Telegram chat to this user
    // so /portfolio etc. work from the bot.
    telegramChatId: {
      type: String,
      default: "",
      index: true,
    ...
    // Password reset — stores SHA-256 hash of the one-time token, never the raw token.
    resetPasswordToken: {
      type: String,
      default: "",
      index: true,
